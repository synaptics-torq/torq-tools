...
SRC="$WD/tests/testdata/onnx_models/tsuki_static_new_fp32_split_stft_final_s50_4s/part_a_pre_stft_4s.onnx"
...
echo "=== Step 20b: Fix Round half-up (banker's rounding → floor(x+0.5)) ==="
python3 scripts/fix_round_half_up.py \
    "$STAGE/step20.onnx" -o "$STAGE/step20.onnx"
fi
...
# python3 scripts/squeeze_batch_matmul.py \
#     -i "$STAGE/step26.onnx" -o "$STAGE/step27.onnx"

# Step 35 (decompose depthwise Conv1D) DISABLED — causes stack allocation
# overflow on distant im2col Concat ops due to TileAndFuse fusion changes.
# Needs broader Concat barrier fix before re-enabling.
#if run_step 35; then
#echo "=== Step 35: Decompose depthwise Conv1D into elementwise (NSS-compatible) ==="
#python3 scripts/decompose_depthwise_conv_elementwise.py \
#    -i "$STAGE/step26.onnx" -o "$STAGE/step35.onnx" --min-kernel 16
#fi

if run_step 30; then
echo "=== Step 30: Conv1D → im2col + Gemm (large Conv1D only, weight >= 512KB) ==="
python3 scripts/conv1d_im2col.py \
...
python3 scripts/eliminate_reducemean_transposes.py \
    -i "$STAGE/step32.onnx" -o "$STAGE/step31.onnx"
fi


cp "$STAGE/step31.onnx" "$STAGE/step36.onnx"
#if run_step 36; then
#echo "=== Step 36: Replace ReduceSum with ReduceMean * N ==="
#python3 scripts/replace_reducesum_with_mean.py \
#    -i "$STAGE/step31.onnx" -o "$STAGE/step36.onnx"

if run_step 36; then
echo "=== Step 36b: Fix rank-2 ReduceMean (from ReduceSum replacement) ==="
python3 scripts/fix_rank2_reducemean.py \
    -i "$STAGE/step36.onnx" -o "$STAGE/step36.onnx"
fi

if run_step 38; then
echo "=== Step 38: Expand Where broadcast inputs (compiler bug workaround) ==="
python3 scripts/expand_where_broadcasts.py \
    -i "$STAGE/step36.onnx" -o "$STAGE/step38.onnx"
fi

if run_step 39; then
echo "=== Step 39: Fix Resize decomposition bf16 precision loss ==="
python3 scripts/fix_resize_frac.py \
    "$STAGE/step38.onnx" -o "$STAGE/step39.onnx"
fi

if run_step 40; then
echo "=== Step 40: Simplify integer Sign chains (remove Clip(-1,1)) ==="
python3 scripts/simplify_int_sign.py \
    "$STAGE/step39.onnx" -o "$STAGE/step40.onnx"
fi

if run_step 41; then
echo "=== Step 41: Extract Gemm bias with fusion barrier (compiler drops bias bug) ==="
python3 scripts/extract_gemm_bias.py \
    "$STAGE/step40.onnx" -o "$STAGE/step41.onnx"
fi

if run_step 42; then
echo "=== Step 42: Extract stft_source chain for ORT ==="
python3 scripts/extract_stft_source_chain.py \
    -i "$STAGE/step41.onnx" -o "$STAGE/step42.onnx" \
    --submodel "$STAGE/stft_source_fp32.onnx" \
    --reference /tmp/acc_sub6/stft_source_submodel.onnx
fi

if run_step 99; then
echo "=== Step 99: Deduplicate node names ==="
python3 scripts/dedup_node_names.py \
    -i "$STAGE/step42.onnx" -o "$OUT"
fi
...
"
# Copy stft_source submodel alongside the output
STFT_SUB="$(dirname "$OUT")/stft_source_fp32.onnx"
cp "$STAGE/stft_source_fp32.onnx" "$STFT_SUB"
echo "stft_source submodel: $STFT_SUB"
