...
def _replace_comparison_single(model, name, op, a_name, b_name, final_output,
    ...
    # Less/GreaterOrEqual use Sub(b, a): positive when a < b
    # Greater/LessOrEqual use Sub(a, b): positive when a > b
    # The OrEqual variants then invert with Sub(1, ...) to get NOT Less / NOT Greater
...
def replace_comparison(model, chain, nodes):
    ...
    ndim = len(out_shape)
    # Pad at the BEGINNING of dim 0 so Slice [1:1+N] recovers the original data
    pads_val = [1] + [0] * (2 * ndim - 1)
