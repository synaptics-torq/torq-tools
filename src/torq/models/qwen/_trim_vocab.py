...
def _is_latin_digit_text(text: str) -> bool:
    ...
    return (
        all(_is_latin_digit(ch) or ch.isspace() for ch in text)
        and any(_is_latin_digit(ch) for ch in text)
    )
...
def _is_punctuation_only_text(text: str) -> bool:
    ...
    return all(
        ch.isspace() or _is_common_punctuation(ch)
        for ch in text
    )
...
def _collect_required_token_ids(
...
def build_qwen_trimmed_vocab_spec(
    tokenizer: Tokenizer,
    tokenizer_json: dict[str, Any],
    config_json: dict[str, Any] | None,
    ...
    *,
...
) -> QwenTrimmedVocabSpec:
    selected_groups = tuple(dict.fromkeys(selected_groups))

    invalid_groups = sorted(
        set(selected_groups)
        - set(QWEN_TRIM_GROUP_CHOICES)
    )

    if invalid_groups:
        raise ValueError(
            f"Invalid Qwen trim-vocab groups {invalid_groups}; "
            f"expected subset of {QWEN_TRIM_GROUP_CHOICES}"
        )

    model_vocab = tokenizer_json["model"]["vocab"]
    model_vocab_size = len(model_vocab)

    base_groups = build_token_groups(
        tokenizer,
        model_vocab_size,
    )
    ...
    kept_special_ids = (
        added_token_ids
        | _collect_required_token_ids(
            config_json,
            tokenizer_json,
        )
    )

    kept_model_ids.update(
        token_id
        for token_id in kept_special_ids
        if 0 <= token_id < model_vocab_size
    )

    kept_model_ids_sorted = tuple(
        sorted(kept_model_ids)
    )

    extra_token_ids = tuple(
        sorted(
            token_id
            for token_id in kept_special_ids
