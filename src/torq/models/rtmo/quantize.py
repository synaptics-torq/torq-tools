...
"""Quantize the (post-processing-stripped) RTMO model to int8 TFLite.

Pipeline: (1) prepare the ONNX — onnxsim folds the neck's dynamic-shape guards
and the exact GELU is replaced with quick-GELU ``y*sigmoid(1.702*y)`` (no Flex
Erf, lowers as scaled SiLU); (2) onnx2tf with the ``tf_converter`` backend
(flatbuffer_direct's strict quantizer rejects the attention act*act MatMul),
rewriting NCHW->NHWC; (3) verify float TFLite vs ONNX (cosine); (4) PTQ on a
representative image set (int8, or int16x8 for higher accuracy).
"""
...
def _replace_exact_gelu_with_quickgelu(model) -> int:
    """Rewrite each exact-GELU (0.5*y*(1+erf(y/sqrt2))) to y*sigmoid(1.702*y); return count."""
    ...
    for erf in [n for n in g.node if n.op_type == "Erf"]:
        ...
        mul_y = sole(add1.output[0], "Mul") if add1 is not None else None
        ...
        g.initializer.append(numpy_helper.from_array(np.array(QUICK_GELU_SCALE, np.float32), scale_name))
...
def preprocess(path, input_size=DEFAULT_INPUT_SIZE, mean=0.0, std=1.0, *, crop=None, flip=False) -> np.ndarray:
    ...
    img = (img[:, :, ::-1].astype(np.float32) - mean) / std  # BGR->RGB
    return img[None].astype(np.float32)


def build_calibration_set(images_dir, n_samples=200, input_size=DEFAULT_INPUT_SIZE, mean=0.0, std=1.0, seed=0) -> list[np.ndarray]:
...
def _add_int8_input(model_bytes: bytes, in_scale: float = 1.0, in_zp: int = -128) -> bytes:
    """Splice an int8 graph input onto an int16-input TFLite model.

    int16x8 mode forbids int8 I/O, so prepend ``int8 ->DEQUANTIZE-> f32
    ->QUANTIZE-> int16`` (QUANTIZE can't go int8->int16 directly). Default
    (1.0, -128) maps pixel-128 to the model's [0,255] range.
    """
    ...
    def opcode(builtin_code):
        for i, oc in enumerate(m.operatorCodes):
            if oc.builtinCode == builtin_code:
                return i
        oc = fb.OperatorCodeT()
        oc.builtinCode, oc.deprecatedBuiltinCode, oc.version = builtin_code, min(builtin_code, 127), 1
...
def quantize_int8(saved_model_dir, calibration_set, out_path, scheme="int8", int8_input_scale=1.0, int8_input_zp=-128) -> Path:
    """PTQ from a SavedModel: 'int8' (full int8), 'int16x8' (int8 weights / int16
    acts), or 'int16x8_int8in' (int16x8 with a spliced int8 image input)."""
    ...
    if scheme == "int8":
        ...
        if scheme == "int16x8_int8in":
            converter.inference_input_type = converter.inference_output_type = tf.int16
    else:
...
# ---- Orchestration / CLI ----
def quantize_rtmo(onnx_path, out_dir, images_dir, input_size=DEFAULT_INPUT_SIZE, n_calib=200, n_verify=16, mean=0.0, std=1.0, scheme="int8") -> dict:
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
...
def add_rtmo_quantize_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("-i", "--onnx", default="models/rtmo/export/rtmo_nopost_fp32.onnx", help="Source fp32 ONNX (post-processing stripped) (default: %(default)s)")
    parser.add_argument("-o", "--out-dir", default="models/rtmo/export/int8", help="Output directory (default: %(default)s)")
    ...
    parser.add_argument("--scheme", choices=["int8", "int16x8", "int16x8_int8in"], default="int8", help="Quantization scheme (default: %(default)s)")
...
def main() -> None:
    ...
    res = quantize_rtmo(args.onnx, args.out_dir, args.images_dir, args.input_size, args.n_calib, args.n_verify, args.mean, args.std, args.scheme)
