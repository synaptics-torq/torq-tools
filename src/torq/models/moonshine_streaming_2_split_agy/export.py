...
from ._graph import MoonshineStreaming2SplitOnnxGraphEditor
from ._inference import MoonshineStreaming2Split
...
class StatefulFusedEncoderWrapper(torch.nn.Module):
    ...
    def __init__(self, model, chunk_len: int):
        ...
        self.F = self.frontend.n_frames // 4
...
class MoonshineStreaming2SplitExporter(OnnxModelExporterBase):
    ...
    def __init__(
        ...
        **edit_args,
        ...
        super().__init__(
            model_dtype,
            True,  # always static for 2-split
            self._config,
    ...
    def _generate_source_onnx(self):
        ...
        self._logger.info(
            "Fused encoder: chunk_len=%d, F=%d, total_la=%d, warmup_chunks=%d",
    ...
    def _make_fused_encoder_static(self, model: onnx.ModelProto) -> onnx.ModelProto:
        editor = MoonshineStreaming2SplitOnnxGraphEditor.from_onnx(
    ...
    def make_static(self):
        ...
        self._logger.info(
            "Applying 2-split static graph edits (F=%d, max_tokens=%d)...",
            F, self._max_tokens,
        )
    ...
    def validate_onnx(self, n_iters: int = 5):
        self._logger.info("Validating exported 2-split ONNX models...")
        runner = MoonshineStreaming2Split.from_onnx(
...
def export_moonshine_streaming_from_args(args: argparse.Namespace):
    ...
    if getattr(args, "chunk_len", None) is None:
        raise ValueError("--chunk-len is required for the 2-split streaming architecture")
    exporter = MoonshineStreaming2SplitExporter(
        args.model_size,
        args.dtype,
...
def main():
    parser = argparse.ArgumentParser(description="Export 2-Split Moonshine Streaming to Torq")
