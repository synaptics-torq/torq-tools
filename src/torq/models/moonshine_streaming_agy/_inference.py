...
def load_moonshine(
    ...
    for m in models:
        if m.name == "preprocessor.onnx" or m.name == "preprocessor.vmfb" or m.name == "preprocessor.tflite":
            preprocessor = m
        elif m.name == "encoder.onnx" or m.name == "encoder.vmfb" or m.name == "encoder.tflite":
            encoder = m
        elif m.name == "decoder.onnx" or m.name == "decoder.vmfb" or m.name == "decoder.tflite":
            decoder = m
        elif m.name == "decoder_with_past.onnx" or m.name == "decoder_with_past.vmfb" or m.name == "decoder_with_past.tflite":
            decoder_with_past = m
    ...
    if is_static:
        if kind == "vmfb":
            ...
            return MoonshineStreamingStatic.from_tflite(
                encoder, decoder, decoder_with_past, model_size, n_threads=n_threads, preprocessor_model=preprocessor
            )
