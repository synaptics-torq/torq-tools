...
try:
    from ._inference import MoonshineStreamingDynamic, MoonshineStreamingStatic, load_moonshine
except ImportError:
...
def load_tokenizer(model_size: str):
    ...
    tokenizer_file = hf_hub_download(f"UsefulSensors/moonshine-streaming-{model_size}", "tokenizer.json")
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Validate and compare ONNX FP32, IREE FP32, and IREE BF16 WER for Moonshine Streaming.")
    ...
    parser.add_argument("--model-size", default="tiny", choices=["tiny", "small"], help="Model size")
