...
Usage:
    python infer_test.py [--model-dir moonshine_streaming_tiny]
...
class MoonshineASRHuggingFace:
    ...
    def __init__(self, model_dir: str):
        ...
        self._torch = torch
        dir_stem = Path(model_dir).name
        repo_id = self._REPO_MAP.get(dir_stem)
        if repo_id is None:
            raise ValueError(
                f"Unknown model-dir '{dir_stem}' for HF mode. "
                f"Expected one of: {list(self._REPO_MAP)}"
            )
...
def main():
    ...
    parser.add_argument("--model-dir", default="moonshine_streaming_tiny", help="ONNX model directory")
