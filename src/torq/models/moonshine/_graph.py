...
class ReplaceDynamicKVCache(OnnxGraphEdit):
    ...
    def __call__(self, node: gs.Node):
        if node.op == "Concat" and node.outputs[0].name in self.output_names:
    ...
    def __call__(self, node: gs.Node):
        if node.op == "MatMul":
            ...
            if a_shape and b_shape and len(a_shape) >= 2 and len(b_shape) >= 2:
                if a_shape[-1] == 1 and b_shape[-2] == 1 and b_shape[-1] == 1:
                    a, b = node.inputs
                    a_shape = getattr(a, "shape", None)
                    b_shape = getattr(b, "shape", None)
                    y = node.outputs[0]
...
class ReplaceInt64FloatCast(OnnxGraphEdit):
    ...
    def __call__(self, node: gs.Node):
        ...
        if node.op == "Cast":
            if _is_integer_type(node.inputs[0].dtype) and node.attrs["to"] in (
                ...
                if not _is_integer_type(inp_dtype):
                    raise ValueError(
...
class MoonshineOnnxGraphEditor(OnnxGraphEditor):
    ...
    def replace_dynamic_kv_cache(
        ...
        self.apply_edit(ReplaceDynamicKVCache(self._graph_name, cur_len, max_tokens, self._graph))
        return self
    ...
    def mask_future_attn_scores(
        ...
        self.apply_edit(MaskFutureAttentionScores(self._graph_name, cur_len, max_tokens, self._graph, self._export_dtype))
        return self
    ...
    def add_curr_len_input(
        ...
        self.apply_edit(AddCurrLenInput(self._graph_name, cur_len))
        return self

    def convert_to_static_index(
        self
    ):
        self.apply_edit(ConvertToStaticIndex(self._graph_name))
        return self
    ...
    def dequantize_projections_matmul(
        ...
        self.apply_edit(DequantizeProjectionsMatMul(self._graph_name, hidden_size, vocab_size, self._export_dtype))
        return self

    def remove_isNaN(
        self
    ):
        self.apply_edit(RemoveIsNaN(self._graph_name))
        return self
    ...
    def move_output_from_concat(
        ...
        self.apply_edit(MoveOutputFromConcat(self._graph_name, pad_len, self._graph))
        return self

    def fold_scalar_matmul(
        self
    ):
        self.apply_edit(FoldScalarMatMul(self._graph_name, self._graph))
        return self
    ...
    def replace_int64_float_cast(
        ...
        self.apply_edit(ReplaceInt64FloatCast(self._graph_name, max_int, self._graph))
        return self
